...
# Require a GitHub username for registry authentication and the package namespace.
: "${GITHUB_USERNAME:?Set GITHUB_USERNAME to your GitHub username}"

# Use the script's directory as the build context, regardless of where it is invoked.
cd -- "$(dirname -- "${BASH_SOURCE[0]}")"

# Set the requested local image name and latest tag.
local_image="${DOCKER_USERNAME}/karsajobs:latest"

# Set the GitHub Container Registry image name using a lowercase namespace.
registry_image="ghcr.io/${GITHUB_USERNAME}/karsajobs:latest"

# Build the local image from the provided Dockerfile and current directory.
docker build --file Dockerfile --tag "$local_image" .

# Add the registry tag needed to publish the image to GitHub Packages.
docker tag "$local_image" "$registry_image"

# Prompt for a token only if GITHUB_TOKEN is unset or empty.
...
printf '%s' "$GITHUB_TOKEN" | docker login ghcr.io --username "$GITHUB_USERNAME" --password-stdin
